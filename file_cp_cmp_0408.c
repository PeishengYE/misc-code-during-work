...
int compare(FILE *file1, FILE *file2, int file_size)
{
	...
	while(1){
		...
		if((nread1 = fread(buf1,1,COMP_BUF_SIZE,file1)) !=COMP_BUF_SIZE){
			printf("fead file1 failed\n");
			nread2 = fread(buf2,1,COMP_BUF_SIZE,file2);
			break;
		}
		if((nread2 = fread(buf2,1,COMP_BUF_SIZE,file2)) !=COMP_BUF_SIZE){
	...
	if(buf_compare(buf1, buf2, nread1) == FAILED){
				printf("FAILED\n");
				return FAILED;
	}else{
				printf("PASS\n");
...
/* get file size */
	...
	pthread_create(&a_thread,NULL,thread_write,NULL);	
