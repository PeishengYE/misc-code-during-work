...
#endif
#if 1//def __TOUCH_TEST_XIE_ADD__
#include "gui_themes.h"
S32 xiex,xiey,xiex1,xiey1,xiewidth,xieheight;
/*
void PEN_CHECK_BOUND_callback(void)
{
	gui_lock_double_buffer();
	gdi_draw_rect(xiex1, xiey1, xiex1+xiewidth,xiey1+xieheight,GDI_COLOR_RED);
	gui_unlock_double_buffer();
	gui_BLT_double_buffer(xiex1, xiey1, xiex1+xiewidth,xiey1+xieheight);
}
BOOL PEN_CHECK_BOUND(S32 x,S32 y,S32 x1,S32 y1,S32 width,S32 height)
{
	xiex = x;
	xiey = y;
	xiex1 = x1;
	...
	xiewidth = width;
	xieheight = height;
        gui_start_timer(100, PEN_CHECK_BOUND_callback);
	return (x) >= (x1) && (y) >= (y1) && (x) < ((x1) + (width)) && (y) < ((y1) + (height))?TRUE:FALSE;
}*/

void xie_test_point_callback(void)
{
	gui_lock_double_buffer();
	gui_push_clip();
	gui_set_clip(xiex-5, xiey-5, xiex+5,xiey+5);
	gdi_draw_solid_circle(xiex,xiey,4,GDI_COLOR_GREEN);
	gdi_draw_solid_circle(xiex,xiey,4,GDI_COLOR_WHITE);
	gdi_draw_solid_circle(xiex,xiey,3,GDI_COLOR_BLACK);
	gdi_draw_solid_circle(xiex,xiey,2,GDI_COLOR_RED);
	gui_pop_clip();
	gui_unlock_double_buffer();
	gui_BLT_double_buffer(xiex-5, xiey-5, xiex+5,xiey+5);
}
void xie_test_touch_from_custom(kal_int32 x,kal_int32 y,kal_int32 z)
{
	xiex = x;
	xiey = y;
	gui_start_timer(300,xie_test_point_callback);
}
...
//    MMI_TRACE(MMI_FW_TRC_G1_FRM, TRC_MMI_FRM_PEN_DRV_EVENT, data.event, pos.x, pos.y, data.time_stamp);
    kal_prompt_trace(MOD_TP_TASK, "CWH %d  %d",pos.x,pos.y);
...
#endif /* __MMI_TOUCH_PANEL_SHORTCUT__ */
    {
        ...
        switch (data.event)
        {
            ...
            case DCL_PEN_DOWN:
		#if 1//ndef __MTK_TARGET__
		{
			xiex = pos.x;
			xiey = pos.y;
			gui_start_timer(400,xie_test_point_callback);
		}
		#endif
                g_pen_cntx.is_pen_down = 1;
